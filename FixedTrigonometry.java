...
public class FixedTrigonometry {
    ...
    private static FixedArithmetic reduceAngle(FixedArithmetic angle) {
        ...
        // Adjust k toward the nearest integer (round-half-to-even is not needed;
        // simple floor + check is sufficient).
        // If remainder ≥ 0.5, increment; if ≤ -0.5, decrement.
        long rem = divided.remainder();  // units of 10^-9
        long halfScale = 500_000_000L;   // 0.5 in units of 10^-9
        if (rem >= halfScale)  k = k + 1;
        if (rem <= -halfScale) k = k - 1;
